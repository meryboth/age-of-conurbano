...
export function createWorld(scene,state){const pickables=[],obstacles=[];const statics=new THREE.Group();scene.add(statics);let seed=81;const rand=()=>{seed=(seed*1664525+1013904223)>>>0;return seed/4294967296;};
 ...
 const tower=new THREE.Group();tower.position.set(30,0,-30);statics.add(tower);for(const x of [-.6,.6])for(const z of [-.6,.6])box(.1,4.6,.1,'#7c8f8c',x,2.3,z,tower);cyl(1,1.4,'#bcc3b4',0,4.7,0,tower,12);cyl(1.08,.08,'#8faba8',0,5.44,0,tower,12);sign('TERO COSECHADO',1.6,.3,0,4.9,1.01,tower,'#aebbb0','#46676c');obstacles.push({x:30,z:-30,width:1.6,depth:1.6});
